...
async function updateStatus(docId) {
  ...
  try {
    ...
    if (!statusInfo.isPending) {
      ...
      logger.debug(`Nothing to update for dok ${docId}.`);
      return null;
    }
  } catch (error) {
        ...
        let updatedDocs = await Promise.all(putPromises)
          .then(data => {
            return data.map(item => {
              if (item.result === "updated") { return true; } else { return false; }
            });
          });

        logger.info(`Updated ${updatedDocs.length} documents.`);
        callback(200);
      } catch (error) {
        logger.error("An error occurred when updating documents.", error);
        callback(500);
      }
    }
  });
